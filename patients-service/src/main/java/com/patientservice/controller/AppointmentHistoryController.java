...
import com.patientservice.entities.PrescriptionHistoryList;
...
public class AppointmentHistoryController {
    ...
    public ResponseEntity<?> get(@PathVariable long pid  , @PathVariable long aId){
        //patientServiceImpl.getPatientById(pId);
        ResponseEntity<?> response = null;
        PrescriptionHistoryList p = restTemplate.getForObject("http://appointment-service/appointment/appointments/"+aId+"/prescriptions", PrescriptionHistoryList.class);
        response = new ResponseEntity<>(p.getPrescriptionList(),HttpStatus.OK);
        return response ;
